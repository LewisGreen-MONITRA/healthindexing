...
def main():
    ...
    filtered = getData()
    for s in filtered['sensor_name'].unique():
       selected_sensor = s
       trend_field = filtered['trend_field']
       for u in filtered['units'].unique():
           selected_unit = u
           sensor_filtered_df = filtered[filtered['sensor_name'] == selected_sensor]
           sensor_filtered_df = sensor_filtered_df[sensor_filtered_df['units'] == selected_unit]
     ...
     # ===== KALMAN FILTER BASED ANOMALY DETECTION =====
           ...
           # Show data head
           print(f"\nDetected Anomalies for Sensor {selected_sensor}:")
